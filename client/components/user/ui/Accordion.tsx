...
function AccordionItem({
  ...
  return (
    <AccordionPrimitive.Item
      ...
      className={cn(
        "bg-primary/20 mb-[1em] rounded-[1em] p-[1em] last:mb-0",
        className,
      )}
      {...props}
    />
...
function AccordionTrigger({
  ...
  return (
    <AccordionPrimitive.Header className="flex">
      ...
      >
        {children}
        <ChevronDownIcon className="text-foreground/60 pointer-events-none shrink-0 translate-y-0.5 transition-transform duration-200" />
      </AccordionPrimitive.Trigger>
...
function AccordionContent({
  ...
  return (
    <AccordionPrimitive.Content
      ...
      {...props}
    >
      <div className={cn("pt-[1em]", className)}>{children}</div>
    </AccordionPrimitive.Content>
  );
}
